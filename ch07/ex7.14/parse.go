...
func parsePrimary(lex *lexer) Expr {
	...
	case '[':
		...
		if lex.token != ']' {
			for {
				array = append(array, parseExpr(lex))
				if lex.token != ',' {
					break
				}
				lex.next() // consume ','
		...
		lex.next() // consume ']'
		if lex.token != '.' {
			msg := fmt.Sprintf("got %s, wnat '.'", lex.describe())
			panic(lexPanic(msg))
		}
		lex.next() // consume '.'
		switch lex.token {
		case scanner.Ident:
			id := lex.text()
			lex.next()
			return arrayOp{id, array}
		}
		msg := fmt.Sprintf("got %s, want operation (e.g. min)", lex.describe())
		panic(lexPanic(msg))
	}
